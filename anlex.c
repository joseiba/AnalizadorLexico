...
FILE *salida;			//salida de los componentes lexicos
char buff[2*TAMBUFF];	// Buffer para lectura de entrada fuente
...
{
    //En caso de un error muestra el error en el archivo
	fprintf(salida,"Lin %d: Error Lexico. %s.\n",numLinea,mensaje);
    //Sigue el proceso 
    sigLex();
}

char* buscarCompLex(int compLex){
	// se crea el array con los componentes lexicos
	char *tok[] = {
		"L_CORCHETE ",
		"R_CORCHETE ",
		"L_LLAVE ",
...
void sigLex()
{
	...
	while((c=fgetc(fuente))!=EOF)
	{
		...
		else if(c=='\n')
		{
			...
			numLinea++;
			//para fomartear el arhivo de salida
			t.compLex='\n';
			t.pe=buscar("\n");
			break;
		}
